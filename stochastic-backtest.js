...
async function fetchDailyAdjusted(ticker) {
    ...
    if (!res.ok)
        throw new Error(`Fetch failed for ${ticker}: ${res.status} ${res.statusText}`);
    const json = await res.json();
    const result = json?.chart?.result?.[0];
    if (!result)
        throw new Error(`No chart result for ${ticker}: ${JSON.stringify(json?.chart?.error)}`);
...
function fullStochastic(weekly) {
    const rawK = weekly.map((bar, i) => {
        if (i < STOCH_K_PERIOD - 1)
            return null;
...
function xirr(cashflows) {
    ...
    const yearsFrom = (d) => (d.getTime() - t0) / (1000 * 60 * 60 * 24 * 365);
    const npv = (rate) => cashflows.reduce((acc, cf) => acc + cf.amount / Math.pow(1 + rate, yearsFrom(cf.date)), 0);
    let lo = -0.99;
    let hi = 10;
    let flo = npv(lo);
...
async function runBacktest(ticker) {
    ...
    const years = (stoch[stoch.length - 1].date.getTime() - bhStart.date.getTime()) /
        (1000 * 60 * 60 * 24 * 365);
    const bhCagr = Math.pow(bhFinalValue / invested, 1 / years) - 1;
    console.log(`\n=== ${ticker} ===`);
    console.log(`Data window used         : ${stoch[0].date.toISOString().slice(0, 10)} -> ${stoch[stoch.length - 1].date.toISOString().slice(0, 10)}`);
    console.log(`Signals (fresh <20 cross): ${signals.length}`);
    console.log(`Signal dates             : ${signals.map((s) => s.date.toISOString().slice(0, 10)).join(", ")}`);
    console.log(`Total invested           : $${invested.toLocaleString()} ($${TRANCHE_USD} x ${signals.length} tranches)`);
    console.log(`Final value              : $${finalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
    console.log(`Tranche total return     : ${(trancheTotalReturn * 100).toFixed(1)}%`);
    console.log(`Tranche money-wtd IRR    : ${(trancheIrr * 100).toFixed(2)}% annualized`);
    console.log(`--- vs. lump-sum buy&hold of $${invested.toLocaleString()} on ${bhStart.date.toISOString().slice(0, 10)} ---`);
    console.log(`Buy&hold final value     : $${bhFinalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
    console.log(`Buy&hold CAGR            : ${(bhCagr * 100).toFixed(2)}% annualized`);
}
async function main() {
